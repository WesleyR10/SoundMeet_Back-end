...
- o que foi especificado em [Features-SoundMeet.md](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/Docs/Features-SoundMeet.md)
- com o que já está implementado no código

Marcações:

- `[x]` Implementado (com link para o código)
...
- [x] Perfil de músico/banda com dados básicos (nome, stage_name, bio, contatos)  
  Implementado em [musician.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/domain/musician.aggregate.ts).  
...
- [x] QR Code permanente vinculado ao perfil do músico (não por apresentação)  
  Implementado em [musician.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/domain/musician.aggregate.ts) (`generateQRCode`).  
...
- [x] Lista de gêneros e instrumentos com validação de negócio  
  Implementado em [musician.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/domain/musician.aggregate.ts) e validadores relacionados.  
...
- [x] Sistema de rating do músico com média e contagem de avaliações  
  Implementado em [musician.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/domain/musician.aggregate.ts) usando VO `Rating`.  
  Regras:  
...
- [~] Analytics detalhados (engajamento, comparação entre eventos, demografia musical)  
  ...
  Ainda **não existem** agregados/relatórios dedicados a analytics de músico conforme detalhado em *Features*.

**Bandas e divisão de ganhos**

- [x] Agregado Band com membros e papéis  
  Implementado em [band.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/domain/band.aggregate.ts).  
  Regras principais:  
...
- [x] Divisão automática de gorjetas entre membros da banda (split igualitário)  
  Implementado em [confirm-tip-payment.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/application/use-cases/confirm-tip-payment/confirm-tip-payment.use-case.ts).  
  Regras:  
...
- [ ] Percentuais customizáveis de split por membro  
  Não implementado: hoje o split é igualitário, sem configuração de percentuais personalizados como previsto em *Monetização Especializada*.

**Busca de músicos para contratação**

- [x] Filtros por nome, e-mail, gêneros, instrumentos e status ativo  
  Implementado em [musician-in-memory.repository.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/musician/infra/db/in-memory/musician-in-memory.repository.ts) e testes associados.  
...
- [x] Cadastro de estabelecimento com endereço e tipo (bar, restaurante, etc.)  
  Implementado em [establishment.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/establishment/domain/establishment.aggregate.ts).  
  Regras:  
...
- [x] QR Code permanente do estabelecimento  
  Implementado em [establishment.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/establishment/domain/establishment.aggregate.ts) (`generateQRCode`).  
...
- [ ] Agenda compartilhada em tempo real com disponibilidade dos músicos  
...
Essas funcionalidades estão descritas em detalhes em *Features-SoundMeet*, mas ainda não possuem agregados/use-cases específicos no código.
...
- [x] Perfil de público com preferências musicais e configurações de descoberta  
  Implementado em [audience.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/domain/audience.aggregate.ts).  
  Regras:  
...
- [x] Validação de dados via notification pattern  
  Implementado em [audience.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/domain/audience.aggregate.ts) e validators correspondentes.  
...
- [x] Pontuação por ações do público (scan, pedidos, acertos, gorjetas, social, etc.)  
  Implementado em [audience.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/domain/audience.aggregate.ts) em conjunto com VO `Points` e `AudiencePoints`.  
  Regras alinhadas com *Gamificação Avançada* em *Features-SoundMeet*:  
  - Scan QR: +10 pts  
...
- [x] Sistema de níveis com benefícios por faixa de pontos  
  Implementado via VO de nível (em Audience e no domínio genérico de Gamification, ver [user-level.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/user-level.vo.ts)).  
  Regras:  
...
- [x] Badges básicos por milestones de interação  
  ...
  - Badge “Sugestor” concedido no primeiro pedido via [make-music-request.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/application/use-cases/make-music-request/make-music-request.use-case.ts).  
...
- [x] Criação de pedido musical pelo público a partir do perfil do músico  
  Implementado em:  
  - [audience.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/domain/audience.aggregate.ts) (`makeMusicRequest`).  
  - [make-music-request.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/application/use-cases/make-music-request/make-music-request.use-case.ts).  
  Regras:  
...
- [x] Votação em músicas/pedidos  
  Implementado em [audience.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/domain/audience.aggregate.ts) (`voteForSong`).  
  Regras:  
...
- [x] Recomendação de músicos com base nas preferências do público  
  Implementado em [recommend-musicians.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/audience/application/use-cases/recommend-musicians/recommend-musicians.use-case.ts).  
...
- [x] Pedido musical ligado a público, músico, música e artista  
  Implementado em [request.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/request/domain/request.aggregate.ts).  
  Regras:  
...
- [x] Aceitar e rejeitar pedidos com validações de estado  
  Implementado em [request.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/request/domain/request.aggregate.ts) (`accept`, `reject`).  
  Regras:  
...
- [x] Anti-spam por similaridade de pedidos  
  Implementado em [request.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/request/domain/request.aggregate.ts) (`isSimilarTo`).  
...
- [x] Pontos por pedido criado e pedido aceito  
  ...
  - [request.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/request/domain/request.aggregate.ts) (métodos que calculam valor em pontos).  
  - [user-points.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/user-points.aggregate.ts) (`makeMusicRequest`, `acceptedMusicRequest`).  
  Regras:  
...
- [x] Entidade Tip com vínculo a público, músico e banda  
  Implementado em [tip.entity.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/domain/tip.entity.ts).  
  Regras:  
...
- [x] Mensagem personalizada com a gorjeta  
  Suportado via campo `message` em [tip.entity.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/domain/tip.entity.ts).  
...
- [x] Carteira financeira para músicos (MusicianWallet)  
  Implementado em [musician-wallet.entity.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/domain/musician-wallet.entity.ts).  
  Regras:  
...
- [x] Registro de transações financeiras (Transaction)  
  Implementado em [transaction.entity.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/domain/transaction.entity.ts).  
  Regras:  
...
- [x] Saque do músico para PIX com validação de saldo  
  Implementado em [withdraw-to-pix.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/payment/application/use-cases/withdraw-to-pix/withdraw-to-pix.use-case.ts).  
  Regras:  
...
- [ ] Planos de assinatura para músicos e estabelecimentos (valores, limites, taxas diferenciadas)  
  Ainda não há agregados específicos de plano, cobrança recorrente ou lógica de pricing implementados, apesar de descritos na seção “Resumo de Monetização” em *Features-SoundMeet*.
...
- [x] Acúmulo de pontos por tipo de ação  
  Implementado em [user-points.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/user-points.aggregate.ts) e [points-source.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/points-source.vo.ts).  
  Regras:  
  - `scanQr` → +10 pts  
...
- [x] Cálculo de nível, progressão e detecção de level up  
  Implementado em [user-points.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/user-points.aggregate.ts) e [user-level.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/user-level.vo.ts).  
  Regras:  
...
- [x] Use-case central para cálculo de pontos  
  Implementado em [calculate-points.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/application/use-cases/calculate-points/calculate-points.use-case.ts).  
...
- [x] Sistema genérico de badges com progresso e desbloqueio  
  Implementado em [user-badge.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/user-badge.aggregate.ts) e [badge-type.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/badge-type.vo.ts).  
  Regras:  
...
- [x] Use-cases para concessão e manipulação de badges  
  Ver [award-badge.use-case.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/application/use-cases/award-badge/award-badge.use-case.ts) e demais use-cases em `application/use-cases`.  
...
- [x] Registro tipado de interações de usuário (UserInteraction)  
  Implementado em [user-interaction.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/user-interaction.aggregate.ts) e [interaction-metadata.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/interaction-metadata.vo.ts).  
...
- [x] Rankings por tipo e período  
  Implementado em [ranking.aggregate.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/ranking.aggregate.ts) e [ranking-type.vo.ts](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/src/core/gamification/domain/value-objects/ranking-type.vo.ts).  
  Regras:  
...
- [~] Algoritmo completo de ranking mensal (Top Fãs, Top Sugestões, Top Discoverers, etc.)  
  Estrutura de `Ranking` e `UserPoints` já está pronta, com use-cases para cálculo e leaderboard; contudo, a lógica fina de cada tipo de ranking (combinações específicas de métricas) ainda pode ser expandida para refletir todos os cenários descritos em *Features-SoundMeet*.
...
Esses itens já estão bem descritos em [Features-SoundMeet.md](file:///home/wesleyr10/Programação/Projetos/SoundMeet/soundmeet-backend/Docs/Features-SoundMeet.md), e o código atual fornece boa parte da base de domínio (QR codes, pedidos, gorjetas, gamificação, rankings). Porém ainda serão necessários novos agregados, use-cases e integrações de infraestrutura para chegar à visão completa da plataforma.
