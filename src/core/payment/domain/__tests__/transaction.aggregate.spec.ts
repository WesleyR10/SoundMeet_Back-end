...
describe("Transaction Aggregate", () => {
  it("should create with net amount calculated and emit event", () => {
    const tx = Transaction.create({ type: TransactionType.TIP, amount: 100, fee: 3, payment_method: PaymentMethod.PIX });
  ...
  it("should complete and fail transitions", () => {
    const tx = Transaction.create({ type: TransactionType.TIP, amount: 50, payment_method: PaymentMethod.PIX });
    tx.complete();
