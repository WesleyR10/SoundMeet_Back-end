...
export class AiCifraUploadFakeBuilder<TBuild = any> extends FakeBuilderBase {
  private _ai_cifra_upload_id: PropOrFactory<AiCifraUploadId> | undefined =
    undefined;
  ...
  private _object_key: PropOrFactory<string> = (_index) =>
    `ai-cifra/${new Uuid().id}/original.mp3`;
  private _upload_method: PropOrFactory<"direct" | "presigned" | "multipart"> =
    (_index) => "direct";
  private _status: PropOrFactory<AiCifraUploadStatus> = (_index) => "uploaded";
  ...
  static anUpload() {
    return new AiCifraUploadFakeBuilder<AiCifraUpload>();
  }

  static theUploads(countObjs: number) {
    return new AiCifraUploadFakeBuilder<AiCifraUpload[]>(countObjs);
  }
  ...
  withId(valueOrFactory: PropOrFactory<AiCifraUploadId>) {
    this._ai_cifra_upload_id = valueOrFactory;
    return this;
  }
  ...
  withStatus(valueOrFactory: PropOrFactory<AiCifraUploadStatus>) {
    ...
    const uploads = new Array(this.countObjs)
      ...
      .map((_, index) => {
        const upload = new AiCifraUpload({
          ai_cifra_upload_id: !this._ai_cifra_upload_id
            ? undefined
            : this.callFactory(this._ai_cifra_upload_id, index),
