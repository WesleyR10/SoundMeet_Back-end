...
import { CreateEstablishmentProfileInput } from "./create-establishment-profile.input";
import { CreateEstablishmentProfileOutput } from "./create-establishment-profile.output";

export class CreateEstablishmentProfileUseCase implements IUseCase<
  CreateEstablishmentProfileInput,
  CreateEstablishmentProfileOutput
> {
  constructor(private readonly establishmentRepo: IEstablishmentRepository) {}

  async execute(
    input: CreateEstablishmentProfileInput,
  ): Promise<CreateEstablishmentProfileOutput> {
    ...
    if (establishment.profile) {
      ...
      notification.addError("Profile already exists", "profile");
      throw new EntityValidationError(notification.toJSON());
    }

    const profile = establishment.ensureProfile(this.toAddress(input.location));
